...
function CreateNewDepositKey(o, i) {
    ...
    var index = database.createNewIndex(function (err, index) {
        if (err) {
            ...
            if (index != null) {
                iotaAccount.getNewAddress(config.seed.toUpperCase(), index, type, function (err, result) {
                    if (err) {
...
function SubmitWithdrawForm(o, i) {
    ...
    iotaAccount.withdraw(toAddress, value, comment, '', function (err, result) {
        if (err) {
...
function GetBalance(o, i) {
    ...
    iotaAccount.getBalances('getBalances', '[' + o + ']', config.threshold, function (err, result) {
        if (err) {
            frame.o = 0;
        }
        else {
...
function GetProviderDetails(o, i) {
    ...
    iota.getNodeInfo('getNodeInfo', function (err, result) {
    ...
    database.getAllAddresses(function (err, result) {
        ...
        var deposits = result.map(function(element) 
        {
            return new Promise(function(resolve, fail) 
            {
                ...
                iotaAccount.getDepositConfirmation(addressArray, function (err2, data) {
    ...
    var totalWalletBalance = new Promise(function(resolve, fail) {
        ...
        database.getAddresseses(function (err, result) {
           if(err){
                ...
                iotaAccount.getWalletBalances(addressArray, function (err, result) {
                    if (err) {
                        res.error = err;
                        res.totalBalance = 0.0;
                        fail(res);
